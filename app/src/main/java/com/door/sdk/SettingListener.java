...
import com.door.activity.InitPwdActivity;
import com.door.entity.AlarmImageInfo;
import com.door.entity.DefenceAreaInfo;
...
public class SettingListener implements ISetting {
   static GateWayListener listener;
    public static void setListener(GateWayListener listener) {
        SettingListener.listener = listener;
    }
    @Override
    ...
    public void ACK_vRetSetInitPassword(int msgId, int result) {
     Intent i = new Intent();
     i.setAction(InitPwdActivity.RET_SET_INIT_PASSWORD);
    ...
    public void ACK_vRetCheckDevicePassword(int msgId, int result, String deviceId) {
        ...
        if(listener!=null){
            if (GateWayService.mp.containsKey(deviceId)) {
                if(result==9997){
                listener.receiveGateWayStatus(2,Integer
                                .parseInt(GateWayService.mp.get(deviceId)),
                        100);
                }else{
                    listener.receiveGateWayStatus(2,Integer
                                    .parseInt(GateWayService.mp.get(deviceId)),
    ...
    public void vRetGetMotionResult(int state) {
        ...
        intent.setAction(GateWaySettingDoor.P2P_MOVE);
    ...
    public void vRetGetInfraredSwitch(int state) {
        Intent intent = new Intent();
        intent.setAction(GateWaySettingDoor.P2P_RF);
        intent.putExtra("state", state);
        MyApplication.getInstance().sendBroadcast(intent);
    }
    ...
    public void vRetSetMotionResult(int result) {
        Intent intent = new Intent();
        intent.setAction(GateWaySettingDoor.P2P_SET_MOVE);
    ...
    public void vRetBindAlarmIdResult(int srcID, int result, int maxCount, String[] data) {
     Intent i = new Intent();
     i.setAction(GateWaySettingDoor.P2P_ALARM);
    ...
    public void vRetSetDevicePasswordResult(int result) {
     Intent i = new Intent();
     i.setAction(GateWaySettingDoor.P2P_SET_PWD);
     i.putExtra("result", result);
...
//        MyApplication.getInstance().sendBroadcast(i);
   ...
   //     RxBus.get().post(RxBUSAction.EVENT_RET_CUSTOM_CMD,info);
    ...
    public void vRetSetInfraredSwitch(int result) {
     Intent intent = new Intent();
     intent.setAction(GateWaySettingDoor.P2P_SET_RF);
     intent.putExtra("result", result);
     MyApplication.getInstance().sendBroadcast(intent);
    }
    ...
    public void vRetSetVisitorDevicePassword(int result) {
     Intent i = new Intent();
     i.setAction(GateWaySettingDoor.P2P_SET_VECTOR_PWD);
     i.putExtra("result", result);
     MyApplication.getInstance().sendBroadcast(i);
    ...
    public void vRetNPCVistorPwd(int pwd) {
        Intent intent = new Intent();
        intent.setAction(GateWaySettingDoor.P2P_VECTOR_PWD);
