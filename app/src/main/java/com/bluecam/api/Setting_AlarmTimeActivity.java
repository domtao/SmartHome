...
public class Setting_AlarmTimeActivity extends BaseActivity implements View.OnClickListener{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        camera.getCameraParam(CameraContants.ParamKey.GET_ALARM_TIME_PARAM_KEY);
    ...
    private void done() {
        ...
        camera.setCameraParam(CameraContants.ParamKey.SET_ALARM_TIME_PARAM_KEY,alarmTimeArr.toString());
        finish();
    }
    ...
    public void onGetParamsEvent(long camHandler, long paramKey, final String params) {
        ...
        if(camHandler == camera.getCamHandler()){
            if(paramKey == CameraContants.ParamKey.GET_ALARM_TIME_PARAM_KEY) {
    ...
    public void onSetParamsEvent(long camHandler, long paramKey, final int nResult) {
        ...
        if(camHandler == camera.getCamHandler()){
            if(paramKey == CameraContants.ParamKey.SET_ALARM_TIME_PARAM_KEY){
                this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if(nResult == 0){
