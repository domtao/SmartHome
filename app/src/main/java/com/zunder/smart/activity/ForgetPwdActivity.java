...
public class ForgetPwdActivity extends Activity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		phone.setText(spf.getString("userName", ""));
...
//				Log.i(TAG, "contentString :" + contentString.length());
				...
				if (contentString.length() > 1)
				{
					...
					{
						tv_countryName.setText(context.getString(R.string.contryNo));
					}
				...
				else
				{
					if (contentString.length() == 0)
					{
						...
						tv_countryName.setText(context.getString(R.string.listselect));
					}
		...
		protected ResultInfo doInBackground(String... params) {
			...
			try {
				result = (ResultInfo) JSONHelper.parseObject(UserServiceUtils
						.updatePassword(phone.getText().toString(), passWord
								.getText().toString()), ResultInfo.class);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	class CheckDataTask extends AsyncTask<String, Void, ResultInfo> {
		...
		protected void onPostExecute(ResultInfo result) {
			...
			if (result != null) {
				...
				alert = new TipAlert(context, getString(R.string.tip),
						getString(R.string.get_check_no));
				alert.show();
			}
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.readBtn:
			...
			if (TextUtils.isEmpty(phone.getText())) {
				Toast.makeText(context, getString(R.string.input_user), Toast.LENGTH_SHORT).show();
				return;
			}
