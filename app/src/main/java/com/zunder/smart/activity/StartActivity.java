...
public class StartActivity extends Activity  implements OnPermissionCallback {
	...
	final String[] permissionArrays = {Manifest.permission.ACCESS_FINE_LOCATION,Manifest.permission.CAMERA,Manifest.permission.RECORD_AUDIO,Manifest.permission.WRITE_EXTERNAL_STORAGE,Manifest.permission.READ_SMS,Manifest.permission.ACCESS_COARSE_LOCATION};
		...
		for(int i=0,length=permissionArrays.length;i<length;i++){
			...
			if (i == length - 1){
				if ("".equals(sb.toString())){
					ToastUtils.ShowError(activity,getString(R.string.open_permit),Toast.LENGTH_SHORT,true);
				}else{
					ToastUtils.ShowSuccess(activity,sb.toString(),Toast.LENGTH_SHORT,true);
	...
	private void doYourThings(){
		...
		if (spf.getInt("wel", 0)==0) {
			WelcomeActivity.startActivity(activity,0);
			finish();
		}else {
