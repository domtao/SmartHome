...
class SimpleWheelDecoration extends WheelDecoration {
    ...
    void drawItem(Canvas c, Rect rect, int position, int alpha, boolean isCenterItem, boolean isVertical) {
        ...
        if(s.length()>10){
            c.drawText(s.substring(0,5), rect.exactCenterX(), rect.exactCenterY() +textHeight, paint);
            c.drawText(s.substring(5,10), rect.exactCenterX(), rect.exactCenterY() - textHeight+5, paint);
            c.drawText(s.substring(10,s.length()), rect.exactCenterX(), rect.exactCenterY() -textHeight+45, paint);
        }else if(s.length()>5){
            c.drawText(s.substring(0,5), rect.exactCenterX(), rect.exactCenterY() +textHeight+10, paint);
            c.drawText(s.substring(5,s.length()), rect.exactCenterX(), rect.exactCenterY() - textHeight+20, paint);
        }else{
    ...
    void drawDivider(Canvas c, Rect rect, boolean isVertical) {
        if (isVertical) {
            ...
            if(dividerSize>120) {
                ...
                c.drawLine(firstX, 20, firstX, 80, dividerPaint);
                c.drawLine(secondX, 20, secondX, 80, dividerPaint);
            }
        }
    }
}
