...
public class DoorPopupWindow implements OnClickListener {
	...
	public DoorPopupWindow(Activity context, String name, String deviceType,
                           int typeId, int deviceId, int actionShow, int functionShow,
                           int actionPramShow, int functionParamShow, GateWay gateWay) {
		super();
...
//				time();
	...
	public void save() {
		...
		if(gateWay!=null){
			gateWay.setGateWayPoint(deviceName+msgTxt.getText().toString());
			MyApplication.getInstance().getWidgetDataBase().updateGateWay(gateWay,gateWay.getGatewayName());
			ToastUtils.ShowSuccess(context,"门铃关联成功", Toast.LENGTH_SHORT,true);
			dismiss();
		}
	}
