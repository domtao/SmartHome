package com.door.sdk;

import android.content.Intent;

import com.door.Utils.RxBUSAction;
import com.door.activity.DoorPlayActivity;
import com.door.entity.AlarmInfo;
...
//        Toast.makeText(MyApplication.getInstance(),"呼叫",Toast.LENGTH_SHORT).show();
    }
    ...
    public void vReject(String deviceId, int reason_code, int exCode1, int exCode2) {
        ...
        intent.setAction(DoorPlayActivity.P2P_REJECT);
    ...
    public void vAccept(int type, int state) {
        ...
        accept.setAction(DoorPlayActivity.P2P_ACCEPT);
        accept.putExtra("type", new int[]{type, state});
        MyApplication.getInstance().sendBroadcast(accept);
    }
    ...
    public void vConnectReady() {
        Intent intent = new Intent();
        intent.setAction(DoorPlayActivity.P2P_READY);
        MyApplication.getInstance().sendBroadcast(intent);
    }
    ...
    public void vAllarmingWitghTime(String srcId, int type, int option, int iGroup, int iItem, int imagecounts, String imagePath, String alarmCapDir, String VideoPath, String sensorName, int deviceType) {
        ...
        //type=2移动 7人体红外 13门铃 24防拆
     //   Log.e("dxsTest","vAllarmingWitghTime.srcId:"+srcId);
        GateWay gateWay= GateWayFactory.getInstance().getGateWay(srcId);
        if(gateWay!=null){
        if(!MyApplication.getInstance().isActivityForeground("com.zunder.smart.jpush.DoorAlarmActivity")&&!MyApplication.getInstance().isActivityForeground("com.door.activity.DoorPlayActivity")){
       if(type==13){
//           Intent intent=new Intent(MyApplication.getInstance(), DoorPlayActivity.class);
//           intent.putExtra("Id",gateWay.getUserName());
//           intent.putExtra("Pwd",gateWay.getUserPassWord());
//           intent.putExtra("type",type);
//           MyApplication.getInstance().startActivity(intent);
           DoorPlayActivity.startActivity(MyApplication.getInstance(),gateWay.getGatewayID(),gateWay.getUserPassWord(),type);
       }else {
         //  DoorAlarmActivity.startActivity(MyApplication.getInstance(), srcId, gateWay.getUserPassWord(), type);
       }
        }
        }
    }
