...
import com.zunder.smart.view.datetime.widget.OnWheelChangedListener;
import com.zunder.smart.view.datetime.widget.OnWheelScrollListener;
import com.zunder.smart.view.datetime.widget.WheelView;
import com.zunder.smart.view.datetime.widget.adapters.AbstractWheelTextAdapter;
import com.zunder.smart.view.datetime.widget.OnWheelChangedListener;
import com.zunder.smart.view.datetime.widget.OnWheelScrollListener;
import com.zunder.smart.view.datetime.widget.WheelView;
import com.zunder.smart.view.datetime.widget.adapters.AbstractWheelTextAdapter;
...
public class DateTimeDialog extends Dialog implements View.OnClickListener {
    ...
    private int  mBlnTimePickerStyle = 0;//时间选择是否显示 全部 1年月日 2时间
    ...
    public DateTimeDialog(Context context, int _mBlnTimePickerStyle,int timeCount, DateChooseInterface dateChooseInterface) {
        super(context);
        this.mContext = context;
        this.mBlnTimePickerStyle=_mBlnTimePickerStyle;
        this.dateChooseInterface = dateChooseInterface;
        mDialog = new Dialog(context, R.style.dialog);
        if(timeCount!=0){
            startHour=timeCount/24;
            endHour=timeCount%24;
        }
        initView();
        initData();
    }
...
//                mDateCalendarTextView.setText(" " + arry_date.get(wheel.getCurrentItem()));
    ...
    private void initSecond() {
        ...
        for (int i = 0; i <= 59; i++) {
            ...
            if (nowMinite == i){
                nowSencondId = arry_second.size() - 1;
            }
        }

        mSecondAdapter = new CalendarTextAdapter(mContext, arry_minute, nowSencondId, MAX_TEXT_SIZE, MIN_TEXT_SIZE);
        mSecondWheelView.setVisibleItems(5);
        mSecondWheelView.setViewAdapter(mSecondAdapter);
        mSecondWheelView.setCurrentItem(nowSencondId);
        mSecondStr = arry_second.get(nowSencondId) + "";
    ...
    private void initView() {
        if(mBlnTimePickerStyle==0) {
            view = LayoutInflater.from(mContext).inflate(R.layout.dialog_date_choose, null);
        }else{
            view = LayoutInflater.from(mContext).inflate(R.layout.dialog_date_choose_320, null);
        }
        ...
        minuteTxt=(TextView)view.findViewById(R.id.minuteTxt);
        secondTxt=(TextView)view.findViewById(R.id.secondTxt);
    ...
    public void setTimePickerGoneStyle() {

        if (mBlnTimePickerStyle==1) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.sure_btn://确定选择按钮监听
                if (mBlnTimePickerStyle==1) {
                    dateChooseInterface.getDateTime(strTimeToYear(mYearStr, mDateStr), mBlnBeLongTerm);
                } else if (mBlnTimePickerStyle==2) {
                    dateChooseInterface.getDateTime(strTimeToTime(mHourStr, mMinuteStr, mSecondStr), mBlnBeLongTerm);
                }else{
                    dateChooseInterface.getDateTime(strTimeToDateAll(mYearStr, mDateStr , mHourStr , mMinuteStr,mSecondStr), mBlnBeLongTerm);
                }
                dismissDialog();
                break;
     ...
     * @return
     */
    private String strTimeToDateAll(String yearStr, String dateStr, String hourStr, String minuteStr, String secondStr) {

        return yearStr.replace("年", "-") + dateStr.replace("月", "-").replace("日", " ")
                + hourStr + ":" + minuteStr+":"+secondStr;
    }

    private String strTimeToYear(String yearStr, String dateStr) {

        return yearStr.replace("年", "-") + dateStr.replace("月", "-").replace("日", "");
    }
    private String strTimeToTime(String hourStr, String minuteStr, String secondStr) {
