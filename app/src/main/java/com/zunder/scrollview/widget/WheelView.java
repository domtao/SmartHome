...
public class WheelView extends ViewGroup {
     ...
     * 中心item颜色
     */
    private int textColorCenter = Color.BLUE;
    /**
     * 分割线颜色
     */
     ...
     * item大小
     */
    private int itemSize = 180;
    /**
     * 分割线之间距离
     */
    private int dividerSize = 180;
    /**
     * 布局方向
     */
...
//                if (selectedPosition != lastSelectedPosition&&wheelDecoration.centerItemPosition<wheelAdapter.getItemCount()) {
                if (selectedPosition != lastSelectedPosition) {
                    listener.onItemSelected(wheelDecoration.centerItemPosition,wheelAdapter.getItemString(wheelDecoration.centerItemPosition));
    ...
    public void setCurrentItem(int position) {
        ...
        if(listener!=null) {
            if(wheelAdapter.adapter!=null&&wheelAdapter.adapter.getItemCount()>0){
                if(position<wheelAdapter.adapter.getItemCount()&&position>=0) {
                    listener.onItemSelected(position, wheelAdapter.adapter.getItem(position));
                }
            }else{
    ...
    public interface OnItemSelectedListener {
        void onItemSelected(int index,String itemName);
    }
