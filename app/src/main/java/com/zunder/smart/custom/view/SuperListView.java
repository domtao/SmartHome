...
public class SuperListView extends ListView implements OnScrollListener {
	...
	public void init(final Context context) {
		header = LayoutInflater.from(context).inflate(R.layout.list_header,
				null);
		footer = LayoutInflater.from(context).inflate(R.layout.list_footer,
				null);
		/*
		 * footer.setOnClickListener(new OnClickListener(){
		 * 
		 * @Override public void onClick(View arg0) {
		 * if(footer.findViewById(R.id
		 * .footProgressBar).getVisibility()==View.VISIBLE) { return ; }
		 * ((ProgressBar)
		 * footer.findViewById(R.id.footProgressBar)).setVisibility
		 * (View.VISIBLE); ((TextView)
		 * footer.findViewById(R.id.footTextTip)).setText
		 * (context.getString(R.string.loading));
		 * refreshListener.onShowNextPage(); }});
		 */
		bannerLayout = (LinearLayout) header.findViewById(R.id.bannerView);
		...
		headerHeight = header.findViewById(R.id.head_reloadLayout)
	...
	public void onScroll(AbsListView arg0, int firstVisiableItem,
			int visibleItemCount, int count) {
			...
			if (lastIndex == count - 1 || lastIndex == count - 2) {
				...
				if (((lastIndex - 1) % 10 == 0 || lastIndex % 10 == 0)
					...
					if (footerState != LOADING_MORE) {
						refreshListener.onShowNextPage((count - 1) / 10 + 1);
					} else {
	...
	private void changeHeaderViewByState() {
		lastUpdatedTextView.setText(context
				...
				+ AppTools.howTimeAgo(context, lastRefreshTime));
		if (isRefreshable) {
			switch (state) {
			case RELEASE_To_REFRESH:
				...
				tipsTextview.setText(context
						.getString(R.string.refresh_release));
	...
	public void showMoreComplete(boolean flag) {
		 ...
		 * ((ProgressBar)
		 * footer.findViewById(R.id.footProgressBar)).setVisibility(View.GONE);
		 * ((TextView)
		 * footer.findViewById(R.id.footTextTip)).setText(context.getString
		 * (R.string.tip_look_more));
		 */
	}
	...
	public void addbannerView(View child) {
		child.setLayoutParams(new LayoutParams(LayoutParams.FILL_PARENT, 429));
		bannerLayout.addView(child);
	}

	public void setVisibility(int gone) {
		footer.findViewById(R.id.footer_progressBar).setVisibility(gone);
		footer.findViewById(R.id.footer_hint).setVisibility(gone);
	}
}
