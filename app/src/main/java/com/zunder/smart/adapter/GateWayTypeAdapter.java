...
public class GateWayTypeAdapter extends BaseAdapter {

	List<GatewayType> list;
	int mSelect = -1;
	Activity context;

	public GateWayTypeAdapter(Activity context, List<GatewayType> list) {
		this.list = list;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (mSelect == list.get(position).getId()) {
			convertView.setBackgroundResource(R.drawable.editsharped);
		//	viewCach.but_text.setTextColor(R.color.red);
		} else {
			convertView.setBackgroundResource(0);
		//	viewCach.but_text.setTextColor(R.color.black);
		}

		viewCach.but_text.setText(list.get(position).getGatewayTypeName());
