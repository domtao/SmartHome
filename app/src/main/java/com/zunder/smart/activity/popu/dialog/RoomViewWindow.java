...
public class RoomViewWindow implements OnClickListener{
	...
	TextView showTxt;
	TextView back;
	public RoomViewWindow(Activity activity, String name, int _selection) {
		super();
		this.activity=activity;
		this.list= RoomFactory.getInstance().getAll();
		if(_selection>=0) {
	...
	private void init(String name) {
		...
		wheel_camera.setItems(RoomFactory.getInstance().getRoomName(1));
