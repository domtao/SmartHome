 ...
 * 
 */
public class SubscribeActivity extends Activity implements
	...
	private List<SubscribeInfo> list = new ArrayList<SubscribeInfo>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		activity = this;
		initView();
		titleTxt.setText(getString(R.string.Travel_reservation));
	}
	public void showDialog(){

		if (warnDialog == null) {
			warnDialog = new WarnDialog(activity, getString(R.string.search));
			warnDialog.setMessage(getString(R.string.get_trip_info)+" 10");
	...
	private void initRightButtonMenuAlert() {
		...
		rightButtonMenuAlert = new RightMenu(activity, R.array.subscribelists,
			...
			public void onItemClick(int position) {
				...
				switch (position) {
					...
					onRefresh();;
						...
						break;
					case 1:
						titleTxt.setText(getString(R.string.histrip));
						EditeMode=1;
				onRefresh();
						break;
					case 2:
						SubscribeAddActivity.startActivity(activity,null,0);
						break;
					case 3:
						clear();
					break;
	...
	public void onRefresh() {
		...
		showDialog();
		if(EditeMode==0) {
			adapter = new SubscribeInfoAdapter(activity, list);
			listView.setAdapter(adapter);
			String result = ISocketCode.setGetSubscribe("getSubscribe",
					AiuiMainActivity.deviceID);
			MainActivity.getInstance().sendCode(result);
		}else {
			subscribeAdapter = new SubscribeAdapter(activity, list);
...
onRefresh();
	...
	public class VoiceAsyncTask extends
			AsyncTask<String, Void, List<SubscribeInfo>> {
		private int pageNum;
		...
		@Override
		protected List<SubscribeInfo> doInBackground(String... params) {
			List<SubscribeInfo> listUser = null;
			try {
				String json = SubscribeServiceUtils.getSubscribe(
						spf.getString("userName", ""), pageNum, 10);
				listUser = (List<SubscribeInfo>) JSONHelper.parseCollection(json,
						List.class, SubscribeInfo.class);
			} catch (Exception e) {
				e.printStackTrace();
				return null;
			}
		...
		@Override
		protected void onPostExecute(List<SubscribeInfo> result) {
			hideProgressDialog();
			refreshLayout.setRefreshing(false);
		...
		public void dispatchMessage(Message msg) {
			switch (msg.what){
				case -2:
					...
					warnDialog.setMessage(getString(R.string.get_trip_info)+" "+(10-index));
					break;
				case -1:
					refreshLayout.setRefreshing(false);
				...
				case 1:
					hideProgressDialog();
					list.add((SubscribeInfo) msg.obj);
	...
	private OnSwipeMenuItemClickListener menuItemClickListener = new OnSwipeMenuItemClickListener() {
		...
		public void onItemClick(Closeable closeable, final int adapterPosition, int menuPosition, int direction) {
			...
			if (direction == SwipeMenuRecyclerView.RIGHT_DIRECTION) {
				final SubscribeInfo device=adapter.getItems().get(adapterPosition);
				switch (menuPosition){
					case 0:
						DialogAlert alert = new DialogAlert(activity);
						alert.init(activity.getString(R.string.is_Del), device.getSubscribeName());
						alert.setSureListener(new DialogAlert.OnSureListener() {
							@Override
							public void onSure() {
								// TODO Auto-generated method stub
								if(EditeMode==0) {
									String result = ISocketCode.setDelSubscribe(device.getId() + "", AiuiMainActivity.deviceID);
									MainActivity.getInstance().sendCode(result);
					...
					case 1:
						if(EditeMode==0){
							SubscribeAddActivity.startActivity(activity,device,0);
						}
						else{
							SubscribeAddActivity.startActivity(activity,device,1);
						}
						break;
					case 2:
						break;
	...
	public class VoiceDelAsyncTask extends AsyncTask<String, Void, ResultInfo> {
		...
		protected ResultInfo doInBackground(String... params) {
			...
			try {
				String json = SubscribeServiceUtils.detSubscribe(Integer
						.parseInt(params[0]));
